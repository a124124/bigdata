...
# [문제4] 1부터 10 까지 출력하는데 3씩 증가 되는 형태로(1 4 7 10)저장되는 벡터를
# 정의하여 v3 변수에 저장한다.(또한 각각 값마다 "A", "B", "C", D" 라는 이름을 부여한다.)
...
m1 <- matrix(1:9, nrow=3,byrow=T, dimnames= list(c('n1','n2','n3')))
m1
...
m2 <- matrix(1:9, nrow=3, byrow=T, dimnames = list(c('row1','row2','row3'),c('col1','col2','col3')))
m2    
...
dim(alpha)
alpha2 <- matrix(append(alpha, c("x","y","z")),nrow=3)
alpha2
...
df2 <- data.frame(col1=c(1:5),col2=c('a','b','c','d','e'),col3=c(6:10))
df2
