...
-- 부서번호가 10, 20인 사원의 이름 업무 근무지 (ename job) 급여순으로 출력
...
--    WHERE E.DEPTNO = D.DEPTNO AND E.DEPTNO IN(10, 20)
    ...
    --JOB 이 SALESMAN 이거나 MANAGER 인 사원의
    --ENAME, SAL, COMM, (SAL*12 +COMM) DNAME, LOC. 연봉 정렬
    
SELECT ENAME, SAL, COMM, SAL * 12 + NVL(COMM, 0) Y, DNAME LOC
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND JOB IN('SALESMAN', 'MANAGER')
    ORDER BY Y DESC;

--COMM 이 NULL 이고 급여가 1200이상인 사원의 ENAME, SAL, HIREDATE, EMPNO, DNAME
-- DNAME, SAL 정렬
...
SELECT ENAME, SAL, HIREDATE, E.DEPTNO, DNAME
    ...
    -- 'NEWYORK' 에서 근무하는 사원의 이름과 급여를 출력
...
SELECT ENAME, DNAME
    ...
    --COMM이 NULL 이 아닌 사원의 이름, 급여, 부서번호, 근무지
